...
class TransactionPDController extends Controller
{
    ...
    {
        $pickup = Transaction::with(['customer', 'outlet', 'detail_transactions'])->where('transaction_type', 'pickup-delivery')->orderBy('transaction_start')->where('transaction_status', 'pending')->get();
        $delivery = Transaction::with(['customer', 'outlet', 'detail_transactions'])->where('transaction_type', 'pickup-delivery')->orderBy('transaction_start')->where('transaction_status', 'on_progress')->get();
        // dd($transaction);
        return view('dashboard.transaction.pickup-delivery.index', [
            // 'pickup_transactions' => $transaction->where('transaction_status', 'pending')->get(),
            // 'delivery_transactions' => $transaction->where('transaction_status', 'on_progress')->get()
    ...
    public function paymentUpdate(Transaction $transaction) {
        ...
        return redirect('/dashboard/transaction/pickup-delivery')->with('success', "Payment: $transaction->invoice_no has been sucessfully updated");
    }
    ...
    public function statusUpdate(Request $request, Transaction $transaction) {
        ...
        return redirect('/dashboard/transaction/pickup-delivery')->with('success', "$transaction->invoice_no status has been sucessfully updated");
    }
